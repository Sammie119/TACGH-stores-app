...
@section('subheader', 'Purchase orders, supplier spend and outstanding balances')
...
@section('content')
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-5 mb-6">
        <form method="GET" action="{{ route('reports.purchases') }}"
            ...
            @if(request()->hasAny(['branch_id', 'supplier_id', 'status', 'date_from', 'date_to']))
            ...
            @can('export reports')
                <a href="{{ route('reports.export.purchases', request()->query()) }}"
                ...
                <a href="{{ route('pdf.purchases-report', request()->query()) }}"
                   target="_blank"
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:24px">
        <div class="bg-white border border-gray-200 rounded-xl p-5">
            <p class="text-xs text-gray-500 uppercase tracking-wide font-medium">Total ordered</p>
            <p class="text-2xl font-semibold text-gray-800 mt-1">
                GHS {{ number_format($summary['total_amount'], 2) }}
            </p>
        </div>
        <div class="bg-white border border-gray-200 rounded-xl p-5">
            <p class="text-xs text-gray-500 uppercase tracking-wide font-medium">Total paid</p>
        ...
        <div class="bg-white border border-gray-200 rounded-xl p-5">
            <p class="text-xs text-gray-500 uppercase tracking-wide font-medium">Purchase orders</p>
            <p class="text-2xl font-semibold text-gray-800 mt-1">
    ...
    <div style="display:grid;grid-template-columns:1.6fr 1fr;gap:20px;margin-bottom:24px">
            ...
            <table class="w-full text-sm" style="border-collapse:collapse">
                ...
                @php
                    $statusColors = [
                        ...
                        'partial'   => 'background:#fee2e2;color:#991b1b',
                ...
                @forelse($byStatus as $row)
                    <tr class="border-b border-gray-50">
                        ...
                        <td class="px-5 py-3 text-right font-medium text-gray-800">
                            GHS {{ number_format($row->total_amount, 2) }}
                        </td>
        ...
        <div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
            <div class="px-5 py-4 border-b border-gray-100">
                <p class="font-semibold text-gray-700">Top suppliers owed</p>
                <p class="text-xs text-gray-400 mt-0.5">Outstanding balance &gt; 0, excludes draft/cancelled</p>
            </div>
            <div class="divide-y divide-gray-50">
                @forelse($topSuppliers as $i => $row)
                    <div class="px-5 py-3" style="display:flex;align-items:center;gap:10px">
                        ...
                        <div style="flex:1;min-width:0">
                            ...
                            <p class="text-xs text-gray-400">
                                {{ $row->order_count }} {{ Str::plural('order', $row->order_count) }}
                            </p>
                        </div>
                        <p class="text-sm font-semibold text-red-600 flex-shrink-0">
                            GHS {{ number_format($row->total_owed, 2) }}
                        </p>
                    </div>
                @empty
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        <table class="w-full text-sm" style="border-collapse:collapse">
            <thead>
            <tr class="bg-gray-50 border-b border-gray-100">
                <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">PO Number</th>
                @if($isSuperAdmin)
                ...
                <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">Ordered by</th>
            ...
            @forelse($purchaseOrders as $po)
                @php
                    $sc = $statusColors[$po->status] ?? 'background:#f3f4f6;color:#374151';
                @endphp
                <tr class="border-b border-gray-50 hover:bg-gray-50">
                    ...
                    @if($isSuperAdmin)
                        <td class="px-5 py-3 text-gray-600 text-xs">{{ $po->branch?->name }}</td>
                    @endif
                    <td class="px-5 py-3 text-gray-700 font-medium">
                        {{ $po->supplier?->name }}
                    </td>
                    <td class="px-5 py-3 text-gray-600 text-xs">{{ $po->createdBy?->name }}</td>
                    <td class="px-5 py-3 text-right font-medium text-gray-800">
                    ...
                    <td class="px-5 py-3 text-right {{ $po->balance_due > 0 ? 'text-red-600 font-medium' : 'text-gray-400' }}">
